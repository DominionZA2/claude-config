...
**Usage:** `/implement-plan {TASK_NUMBER} [optional additional prompt]`
...
- Example: `/implement-plan ACR-678 Pay special attention to performance implications`
...
- Verify the plan is complete and implementable:
  ...
  - If the plan is incomplete or unclear, inform the user: "Plan appears incomplete or unclear. Please review and update `{TASK_NUMBER}-implementation-plan.md` or run `/deliberate-plan-with-codex {TASK_NUMBER}` to refine the plan."
...
- Display a summary of implementation:
  ...
  - **Next Steps:** Suggest running `/deliberate-code-review-with-codex {TASK_NUMBER}` to review the implementation
