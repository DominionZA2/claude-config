# Jira Task Planning Command

Analyze a Jira task and create a detailed implementation plan.
...
**Usage:** `/cosoft-jira-02-plan {TASK_NUMBER} [optional additional prompt]`
...
- Example: `/cosoft-jira-02-plan ACR-678 Pay special attention to performance implications`
- Example (exclude tests): `/cosoft-jira-02-plan ACR-678 exclude tests` or `/cosoft-jira-02-plan ACR-678 no tests needed`
...
- Verify that the task folder exists: `.temp/{TASK_NUMBER}/`
  - **If the task folder does not exist:** Inform the user that the task folder is not found and that they should run the setup command first (`/cosoft-jira-01-setup`)
...
- Finish with an explicit success line so the team knows what to run next: `✅ Implementation plan ready. Next step: /cosoft-jira-03-deliberate-plan-codex {TASK_NUMBER}`
