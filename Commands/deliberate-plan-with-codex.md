...
**Usage:** `/deliberate-plan-codex {TASK_NUMBER} [optional additional prompt]`
...
- Example: `/deliberate-plan-codex ACR-678 Pay special attention to performance implications`
...
- Determine the project root and verify the task folder exists:
  ...
  - If the folder does not exist, inform the user: "Task folder `.temp/{TASK_NUMBER}/` not found. Please run `/setup-task {TASK_NUMBER}` first."
...
- When agreement is reached, end with a clear success line so the team knows what to run next, e.g. `✅ Plan deliberation complete. Next step: /plan {TASK_NUMBER}`
...
- **Task folder not found**: If the task folder is not found, inform user: "Task folder `.temp/{TASK_NUMBER}/` not found. Please run `/setup-task {TASK_NUMBER}` first." and STOP
- **Plan not found**: If `{TASK_NUMBER}-implementation-plan.md` doesn't exist, inform user: "No `{TASK_NUMBER}-implementation-plan.md` file found. Please run `/plan-task {TASK_NUMBER}` first to create an implementation plan." and STOP
