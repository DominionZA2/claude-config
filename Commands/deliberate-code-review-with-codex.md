...
### CRITICAL: AUTOMATIC DELIBERATION LOOP

**DO NOT ASK THE USER IF YOU SHOULD CONTINUE WITH CODEX. The deliberation loop MUST continue automatically until consensus is reached or limits are hit.**

- **NEVER** ask the user "Should I continue with Codex?" or "Should I proceed with deliberation?"
- **ALWAYS** automatically proceed to the next Codex consultation round when there are recommendations still in deliberation
- **ONLY** stop when:
  - Consensus is reached (all recommendations agreed/rejected, no new issues)
  - Deliberation limits are hit (3 AI rounds or 6 total rounds)
- The AI and Codex must go back and forth automatically until they agree on all changes
- If you find yourself wanting to ask the user, instead automatically continue the deliberation loop
...
**CRITICAL: This section runs automatically as part of the deliberation loop. DO NOT ask the user for permission to proceed.**
...
**CRITICAL: DO NOT ASK THE USER. AUTOMATICALLY CONTINUE THE DELIBERATION LOOP.**
...
- Check if there are recommendations still in deliberation:
  - If there are `[IN DELIBERATION]` or `[PENDING]` recommendations:
...
**CRITICAL: This phase runs automatically when there are agreed recommendations. DO NOT ask the user for permission.**

- **Only proceed if there are AGREED recommendations:**
  - If all recommendations are still `[IN DELIBERATION]` or `[PENDING]`:
    - Inform user: "No agreed recommendations yet. Continuing deliberation..."
    - Go back to ROUND N+1 (repeat from "ROUND N: CODEX CONSULTATION")
  - If some recommendations are `[AGREED]` → Implement agreed ones, then continue deliberating on rest after implementation
...
- If new recommendations found:
  ...
  - Inform user: "Post-implementation review complete. New recommendations found. Continuing deliberation..."
  - Go back to ROUND N+1 (repeat from "ROUND N: CODEX CONSULTATION")
...
**CRITICAL: DO NOT ASK THE USER. AUTOMATICALLY CONTINUE DELIBERATION IF CONSENSUS NOT REACHED.**
...
- To verify consensus:
  ...
  - If there are any `[PENDING]` or `[IN DELIBERATION]` recommendations:
    - Inform user: "Consensus not yet reached. Continuing deliberation..."
    - Go back to ROUND N+1 (repeat from "ROUND N: CODEX CONSULTATION")
